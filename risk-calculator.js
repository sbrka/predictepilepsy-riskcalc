...
(function () {
  ...
  class RiskCalculator extends HTMLElement {
      ...
      preds.forEach((p, i) => {
        if (p.type === "slider") {
          ...
          const valBox = p.entry
            ? `<span class="pill" id="ms${i}v" style="padding:2px 6px;display:inline-flex;align-items:center;gap:4px;flex:0 0 auto;white-space:nowrap"><input type="number" inputmode="decimal" class="msentry" data-si="${i}" value="${disp}" min="${p.vals ? p.vals[0] : mn}" max="${p.vals ? p.vals[p.vals.length - 1] : mx}" step="${p.vals ? "any" : (p.step || 1)}" aria-label="${attr(p.name)} value" style="width:52px;border:0;outline:0;background:rgba(255,255,255,.9);border-radius:6px;padding:2px 4px;font:inherit;color:var(--azure-deep);text-align:right">${p.unit ? `<span style="opacity:.85">${esc(p.unit)}</span>` : ""}</span>`
            : `<span class="pill" id="ms${i}v">${disp}${p.unit ? " " + esc(p.unit) : ""}</span>`;
          rail += `<div class="field"><div class="flabel" style="justify-content:space-between;gap:8px"><span style="min-width:0">${esc(p.name)}${p.assess ? ` <button class="info-dot" data-info="${attr(p.assess)}" aria-label="About ${esc(p.name)}">i</button>` : ""}</span>${valBox}</div>
    ...
    /* ---------------- MARKERS (independent risk markers, NOT a combined model) ---------------- */
    _renderMarkers() {
    ...
    _bindChrome(root, back) {
      ...
      if (tabs) tabs.addEventListener("click", (e) => { const b = e.target.closest("button"); if (!b) return; tabs.querySelectorAll("button").forEach((x) => x.classList.toggle("on", x === b)); const cl = b.dataset.view === "classic"; root.querySelector("#grid").style.display = cl ? "none" : "grid"; root.querySelector("#classicview").style.display = (cl && !ext) ? "flex" : "none"; const bi2 = root.querySelector("#belowinfo"); if (bi2 && bi2.dataset.has) bi2.style.display = cl ? "none" : "block"; const ctId = this.getAttribute("classic-target"); const ct = ctId ? document.getElementById(ctId) : null; if (ct) { ct.style.display = cl ? "block" : "none"; const foot = root.querySelector("#foot"); let ed = document.getElementById("rc-ext-disc-" + ctId); if (cl) { if (foot) foot.style.display = "none"; if (!ed) { ed = document.createElement("div"); ed.id = "rc-ext-disc-" + ctId; ed.style.cssText = "max-width:1180px;margin:18px auto 28px;padding:15px 18px;background:#eef6fe;border:1px solid #cfe4fb;border-radius:12px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:12.5px;line-height:1.65;color:#5c6b7a"; ed.innerHTML = `<b style="color:#135ba8;display:block;margin-bottom:6px;font-size:14px;font-family:ui-serif,Georgia,serif">Disclaimer</b><p style="margin:0 0 7px">${DISCLAIMER_BODY}</p><p style="margin:0;color:#8a97a4">&copy; 2025, SeLECT consortium &middot; <a href="mailto:select@usz.ch" style="color:#135ba8;font-weight:600;text-decoration:none">select@usz.ch</a></p>`; ct.parentNode.insertBefore(ed, ct.nextSibling); } ed.style.display = "block"; } else { if (foot) foot.style.display = ""; if (ed) ed.style.display = "none"; } } this.dispatchEvent(new CustomEvent("riskcalc:view", { bubbles: true, detail: { view: cl ? "classic" : "new" } })); });
